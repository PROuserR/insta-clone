...
const PostBody = ({ carousel_id, post_body, heart_id }) => {
      ...
      if (index == 0) {
        photoesTemp.push(
          <div className="carousel-item active" key={index}>
            <img src={`https://prouserr.pythonanywhere.com/media/${post_body.photoes[index]}`} className="post_img" alt="..." />
          </div>
        );
      } else {
        photoesTemp.push(
  ...
  const toggle_heart = async () => {
    fetch(`https://prouserr.pythonanywhere.com/api/add_like/${post_body.post_id}`, {
      method: 'POST',
      headers: {
