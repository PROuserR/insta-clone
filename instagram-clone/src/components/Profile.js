...
const Profile = ({ myProfile, user_id }) => {
  ...
  let getProfile = async () => {
    if(myProfile)
    {
      let res2 = await fetch(`http://127.0.0.1:8000/api/get_profile/${localStorage.getItem('my_id')}`);
      ...
      if(parseInt(user_id) === parseInt(localStorage.getItem('my_id'))){
        ...
        let res = await fetch(`http://127.0.0.1:8000/api/get_profile/${user_id}`);
  ...
  let followUser = async () => {
    await fetch(`http://127.0.0.1:8000/api/follow_user/${localStorage.getItem('my_id')}/${user_id}`, {
        method:'POST', 
        headers:{
  ...
  let delete_story = async () => {
    fetch(`http://127.0.0.1:8000/api/delete_story/${localStorage.getItem('my_id')}`, {
      method: 'DELETE',
      headers: {
        'X-CSRFToken': csrftoken,
      }
  ...
  let my_story_existed = async () => {
    let res = await fetch(`http://127.0.0.1:8000/api/get_my_story/${localStorage.getItem('my_id')}`);
    let data = await res.json();
    if(data.length != 0)
